            ...
            <li v-for="(v, i) in this.templates" v-bind:key="v.template.api_name" class="rounded-md mb-4 relative">
                ...
                <div class="flex-1 flex items-center justify-between border border-gray-200 bg-white rounded-md truncate py-1.5">
                    <div class="flex-1 px-4 py-2 text-sm truncate flex items-center">
                        <button @click="setSelected(v.template)" type="button" class="flex-shrink-0 group relative rounded-full inline-flex items-center justify-center h-5 w-10 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mr-4" role="switch" aria-checked="false">
                            <span class="sr-only">Use setting</span>
                            <span aria-hidden="true" class="pointer-events-none absolute bg-white w-full h-full rounded-md"></span>
                    ...
                    <div class="flex-shrink-0 pr-2">
                        <button class="w-8 h-8 bg-white inline-flex items-center justify-center text-gray-400 rounded-full bg-transparent hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            ...
                            <svg @click="setAccordion(v.template.api_name)" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path class="transition-all" v-show="accordion[v.template.api_name]===false" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                <path class="transition-all" v-show="accordion[v.template.api_name]===true" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                            </svg>
                        </button>
                    </div>
                </div>
                <div :class="(accordion[v.template.api_name] ? 'accordion-body-open ' : 'accordion-body-close ') + 'accordion border-gray-200 p-4 border rounded-md mt-1 space-y-4'">
                    <div>
                    ...
                    <div v-if="v.template.dashboards" :class="(v.template.dashboards.length > 0 ? '' : 'margin-0')">
                        <h2 v-show="v.template.dashboards.length > 0" class="text-sm font-medium text-gray-500">Dashboards</h2>
                        <ul :class="(v.template.dashboards.length > 0 ? 'mt-2 ' : '') + 'leading-8'">
                            <li v-for="(va, ib) in v.template.dashboards" v-bind:key="va+ib" class="inline-block mr-2">
                    ...
                    <div v-if="v.template.datasets" :class="(v.template.datasets.length > 0 ? '' : 'margin-0')">
                        <h2 v-show="v.template.datasets.length > 0" class="text-sm font-medium text-gray-500">Datasets</h2>
                        <ul :class="(v.template.datasets.length > 0 ? 'mt-2 ' : '') + 'leading-8'">
                            <li v-for="(va, ib) in v.template.datasets" v-bind:key="va+ib" class="inline-block mr-2">
                                <div class="flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div v-if="v.template.tags" :class="(v.template.tags.length > 0 ? '' : 'margin-0')">
                        <h2 v-show="v.template.tags.length > 0" class="text-sm font-medium text-gray-500">Tags</h2>
                        <ul :class="(v.template.tags.length > 0 ? 'mt-2 ' : '') + 'leading-8'">
                            <li v-for="(va, ib) in v.template.tags" v-bind:key="va+ib" class="inline">
