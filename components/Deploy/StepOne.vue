            ...
            <li v-for="(v, i) in this.filteredDeployTemplates" v-bind:key="v.api_name" class="rounded-md mb-4 relative">
                ...
                <div class="flex-1 flex items-center justify-between border border-gray-200 bg-white rounded-md truncate py-1.5">
                    <div class="flex-1 px-4 py-2 text-sm truncate flex items-center">
                        ...
                        <span @click="setAccordion(v.api_name)" class="cursor-pointer text-gray-900 font-medium hover:text-gray-600">{{v.label}}</span>
                    </div>
                    <div class="flex-shrink-0 pr-2">
                        <button @click="setAccordion(v.api_name)" class="w-8 h-8 bg-white inline-flex items-center justify-center text-gray-400 rounded-full bg-transparent hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
